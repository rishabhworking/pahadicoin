...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 5454
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 4545
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 15454
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 14545
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 5454
	%{_sbindir}/semanage port -d -p tcp 4545
	%{_sbindir}/semanage port -d -p tcp 15454
	%{_sbindir}/semanage port -d -p tcp 14545
	for selinuxvariant in %{selinux_variants}; do
